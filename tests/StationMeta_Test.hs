...
import StationMeta as SM (StationMeta (..), StationMetaAvailability (..), StationMetaInformation (..), mergeAll)
...
sisPerm = tail sis ++ [head sis]

mergeTest :: [Test]
mergeTest =
  [ "same same" ~: do
      sort sis ~=? sort sisPerm,
    "mergeTest1" ~: do
      let metas = mergeAll sis sss
      sms ~=? metas,
    "mergeTest2" ~: do
      let metas = mergeAll sisPerm sss
      sms ~=? metas
  ]
